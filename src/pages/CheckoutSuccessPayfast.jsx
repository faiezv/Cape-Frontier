...
const CheckoutSuccessPayfast = () => {
  ...
  const [paymentMessage] = useState(
    "Your booking is confirmed. Cape Frontier will confirm final pickup and vehicle details via WhatsApp."
  );

  // Load stored checkout data (this is the source of truth)
  const storedCheckout = useMemo(() => getStoredCheckout(), []);
  const bookingReference = storedCheckout?.bookingReference || "Pending confirmation";
  const emailPayload = storedCheckout?.emailPayload;

  // Immediately send email when component mounts
  useEffect(() => {
    if (!emailPayload) {
      console.error("No email payload found in sessionStorage");
      setEmailStatus("failed");
      return;
    }

    const ref = bookingReference;
    if (!ref || ref === "Pending confirmation") {
      console.error("No valid booking reference");
      setEmailStatus("failed");
      return;
    }

    const emailSentKey = `cape-frontier-email-sent:${ref}`;
    if (sessionStorage.getItem(emailSentKey) || emailSentRef.current) {
      setEmailStatus("sent");
      return;
    }
    ...
    if (
      ...
      console.error("Missing required email fields", emailPayload);
      setEmailStatus("failed");
      return;
    }
    ...
    sendEmail();
  }, [emailPayload, bookingReference]);

  // Scroll to top and start Lenis
  useEffect(() => {
    window.scrollTo(0, 0);
    if (window.lenis) {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.to(glowLeftRef.current, { x: 18, y: -16, scale: 1.05, duration: 5, repeat: -1, yoyo: true, ease: "sine.inOut" });
      gsap.to(glowRightRef.current, { x: -18, y: 18, scale: 1.08, duration: 6, repeat: -1, yoyo: true, ease: "sine.inOut" });
      gsap.to(badgeRef.current, { y: -4, duration: 2.4, repeat: -1, yoyo: true, ease: "sine.inOut" });
    }, pageRef);
    return () => ctx.revert();
  }, []);
  ...
  // Data for UI
  const tour = storedCheckout?.tour;
  const bookingDetails = storedCheckout?.bookingDetails;
  const selectedCurrency = storedCheckout?.selectedCurrency || "ZAR";
  const totalAmountLabel = storedCheckout?.totalAmountLabel;
  const pickupLocation = getPickupLocation(bookingDetails);
  const pickupTime = getPickupTime(bookingDetails);
  const isPrivate = Boolean(bookingDetails?.isPrivate);
  const isCustom = Boolean(bookingDetails?.isCustom);
  const participants = bookingDetails?.participants || 1;
  const ccParticipantEmails = normalizeEmails(bookingDetails?.ccParticipantEmails);
                ...
                <div className="mt-4 rounded-[1.5rem] bg-stone-50 p-4">
                  <div className="flex items-start gap-3">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-white shadow-sm">
                      <img src="/icons/mapPin.png" alt="" className="h-5 w-5 object-contain" />
                    </span>
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">
                        Pickup location
                      </p>
                      <p className="mt-1 text-sm font-bold text-slate-900">{pickupLocation}</p>
...
export default CheckoutSuccessPayfast;